...
class Dashboard extends CI_Controller{
			...
			foreach($sheet as $row){
				if($numrow>1){
					if($row['A']==""){
						...
						$cekdata2=$this->m_data->cekforexcel2($row['C'],$row['D'],$row['G'])->num_rows();
							if($cekdata2>0)
							{
								$false[] = 1;
								continue;
	...
	Public function getdatasitesjprocess() {
		...
		$data = $tahun.$periode;
		$result = $this->m_data->dtb_datasite($kdsite,$data);
	...
	Public function updatedtbapprove($nosj)
	{
	...
	Public function notifemaillate()
	{
		return $this->m_data->kirimemail();
	...
	Public function getsjtambahanfromsj()
	{
		header("Access-Control-Allow-Origin: *");
		header('Content-Type: application/json');
		$sj = $this->input->post('no_sj');
		$result = $this->m_data->tampilnosjforsjtambahan($sj);
		echo json_encode($result);
	}
	...
	Public function exportreportho()
	{
		...
		$excel->setActiveSheetIndex(0)->setCellValue('U3', "NO SJ TAMBAHAN");
		$excel->setActiveSheetIndex(0)->setCellValue('V3', "PO TAMBAH");
		$excel->setActiveSheetIndex(0)->setCellValue('W3', "STEMPEL SJ");
		$excel->setActiveSheetIndex(0)->setCellValue('X3', "STEMPEL DTB");
		...
		$excel->getActiveSheet()->getStyle('Q3')->applyFromArray($style_sel);
		...
		foreach($laporan as $data){ 
			$excel->getActiveSheet()->getStyle("R".$numrow)->applyFromArray($link_style_array);
			$excel->getActiveSheet()->getStyle("S".$numrow)->applyFromArray($link_style_array);
			$excel->getActiveSheet()->getStyle("T".$numrow)->applyFromArray($link_style_array);
			$excel->getActiveSheet()->getStyle("F".$numrow)->applyFromArray($link_style_array);
			$excel->getActiveSheet()->getStyle("I".$numrow)->applyFromArray($link_style_array);
			$excel->getActiveSheet()->getStyle("U" . $numrow)->applyFromArray($link_style_array);
			...
			$excel->setActiveSheetIndex(0)->getCell('F' . $numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/sj/' . $data->dok_sj);
			...
			$excel->setActiveSheetIndex(0)->getCell('I' . $numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/dtb/' . $data->dok_dtb);
			...
			$excel->setActiveSheetIndex(0)->setCellValue('P'.$numrow, $data->qty_terima);
			if($data->selisih=="selisih")
			{
				$ceklis='✓';
			}
			else {
			...
			$excel->setActiveSheetIndex(0)->setCellValue('Q'.$numrow, $ceklis);
			$excel->setActiveSheetIndex(0)->setCellValue('R'.$numrow, $data->no_basp);
			$excel->setActiveSheetIndex(0)->getCell('R'.$numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/basp/'.$data->dok_basp);
			$excel->setActiveSheetIndex(0)->setCellValue('S'.$numrow, $data->no_bppr);
			$excel->setActiveSheetIndex(0)->getCell('S'.$numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/bppr/'.$data->dok_bppr);
			$excel->setActiveSheetIndex(0)->setCellValue('T'.$numrow, $data->no_dtb2);
			$excel->setActiveSheetIndex(0)->getCell('T' . $numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/dtb2/' . $data->dok_dtb2);
			$excel->setActiveSheetIndex(0)->setCellValue('U' . $numrow, $data->no_sj2);
			$excel->setActiveSheetIndex(0)->getCell('U' . $numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/sj2/' . $data->dok_sj2);
		...
		$excel->getActiveSheet()->getColumnDimension('O')->setWidth(7);
		$excel->getActiveSheet()->getColumnDimension('P')->setWidth(7);
		$excel->getActiveSheet()->getColumnDimension('Q')->setWidth(7); 
...
if ($statussj == "sesuai") {
    ...
    if (!$upload_sj && !$upload_dtb) {
        ...
        echo 'upload file sj  Error : ' . $this->sjupload->display_errors() . '<br/>';
        echo 'upload file dtb Error : ' . $this->dtbupload->display_errors() . '<br/>';
    }
...
} elseif ($statussj == "selisih kurang") {
    $pcode = $this->input->post('pcode');
    $pcodename = $this->input->post('pcodename');
...
$updatedeletetransaksi = $this->m_data->deletesjtransaksi($nosjbaruedit);
    ...
    if (!$upload_sj && !$upload_dtb && !$upload_bppr && !$upload_basp) {
        $sj_data = $this->sjupload->data();
        print_r($sj_data);
        ...
        echo 'sj upload Error : ' . $this->sjupload->display_errors() . '<br/>';
        echo 'dtb upload Error : ' . $this->dtbupload->display_errors() . '<br/>';
        echo 'bppr upload Error : ' . $this->bpprupload->display_errors() . '<br/>';
        echo 'basp upload Error : ' . $this->baspupload->display_errors() . '<br/>';
    }
    for ($i = 0; $i < count($pcode); $i++) {
        $this->m_data->uploadTbsk($iddepo, $pcode[$i], $nosjbaruedit, $nodtbbaruedit, $nobpprbaruedit, $nobaspbaruedit, $qtysjedit[$i], $qtyskedit[$i], $qtysledit[$i], $statussj, $tglinputdtb, $tglsekarang, $iduser);
    }
...
$updatedeletetransaksi = $this->m_data->deletesjtransaksi($nosjbaruedit);
    ...
    if (!$upload_sj && !$upload_dtb && !$upload_dtb2 && !$upload_bppr && !$upload_basp) {
        ...
        echo 'sj upload Error : ' . $this->sjupload->display_errors() . '<br/>';
        echo 'dtb upload Error : ' . $this->dtbupload->display_errors() . '<br/>';
        echo 'dtb tambahan upload Error : ' . $this->dtb2upload->display_errors() . '<br/>';
        echo 'basp upload Error : ' . $this->baspupload->display_errors() . '<br/>';
    }
    ...
    $this->m_data->uploaddataTbsl2($nosjbaruedit, $namasj, $namadtb, $namadtb2, $namabasp, $tglsekarang, $iduser);
    $this->m_data->masukapprove2($nosjbaruedit);
    $cekstatuslebih = $this->m_data->cekyanglebih($iddepo, $nosjbaruedit);
    if ($cekstatuslebih) {
        $this->session->set_flashdata('berhasilupdate', 'NO SJ :' . $nosjbaruedit . ' BERHASIL DIPROSES!');
        redirect('dashboard/surat_jalan_processed');
    } else {
...
Public function exportreportarea()
{
	...
	$excel->setActiveSheetIndex(0)->setCellValue('A1', "Output Report Area"); 
	...
	$numrow = 4; 
	foreach($row as $data){ 
		$excel->getActiveSheet()->getStyle("D" . $numrow)->applyFromArray($link_style_array);
		$excel->getActiveSheet()->getStyle("G" . $numrow)->applyFromArray($link_style_array);
		$excel->getActiveSheet()->getStyle("P" . $numrow)->applyFromArray($link_style_array);
		$excel->getActiveSheet()->getStyle("Q" . $numrow)->applyFromArray($link_style_array);
		$excel->getActiveSheet()->getStyle("R" . $numrow)->applyFromArray($link_style_array);
		...
		$excel->setActiveSheetIndex(0)->getCell('D' . $numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/sj/' . $data->dok_sj);
		...
		$excel->setActiveSheetIndex(0)->getCell('G' . $numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/dtb/' . $data->dok_dtb);
		...
		$excel->setActiveSheetIndex(0)->setCellValue('N'.$numrow, $data->qty_terima);
		if ($data->selisih == "selisih") {
				$ceklis = '✓';
			} else {
				$ceklis = '';
			}
		$excel->setActiveSheetIndex(0)->setCellValue('O'.$numrow, $ceklis);
		$excel->setActiveSheetIndex(0)->setCellValue('P'.$numrow, $data->no_basp);
		$excel->setActiveSheetIndex(0)->getCell('P' . $numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/basp/' . $data->dok_basp);

		$excel->setActiveSheetIndex(0)->setCellValue('Q'.$numrow, $data->no_bppr);
		$excel->setActiveSheetIndex(0)->getCell('Q' . $numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/bppr/' . $data->dok_bppr);

		$excel->setActiveSheetIndex(0)->setCellValue('R'.$numrow, $data->no_dtb2);
		$excel->setActiveSheetIndex(0)->getCell('R' . $numrow)->getHyperlink()->setUrl('http://192.168.35.160/pma_dev/pmaterimabarang/pdf/dtb2/' . $data->dok_dtb2);
...
Public function surat_jalan_tambahan()
{
	// $data['datapcode']=$this->m_data->tampilpcode();
	// $data['datasj']=$this->m_data->tampilsuratjalantambahan();
	$data['datagreg']=$this->m_data->tampildatagreg();
	$data['content']="contentsjtambahan";
	$this->load->view('main_page',$data);
}

Public function getDatasjtambahan()
{
	header("Access-Control-Allow-Origin: *");
	header('Content-Type: application/json');
	$grupregion=$this->input->post('txtgrupregion');
	...
	$result = $this->m_data->sjtambahan($tglawal,$tglakhir,$principal,$grupregion,$region,$area);
	echo json_encode($result);
}

Public function getsjtambahan()
{
	header("Access-Control-Allow-Origin: *");
	header('Content-Type: application/json');
	$sjtambahan=$this->input->post('sjtmbhcek');
	$isi = $this->m_data->detailsjtambahan($sjtambahan)->result();
	echo json_encode($isi);
}

Public function updatesjtambahan()
{
	$nosjganti=$this->input->post('nosjbaru');
	$nosjawal=$this->input->post('nosjcad');
	$kdsap2 = $this->input->post('sap');
	$filesj = $_FILES['filesj2']['name'];
	$datesekarang = date("Ymd");
	$jamsekarang = date("His");
	$namafile = $kdsap2 . "_SJ_TAMBAHAN_" . $datesekarang . "_" . $jamsekarang . ".pdf";
	$config['file_name'] = $namafile;
	$config['upload_path'] = './pdf/sj2/';
	$config['allowed_types'] = 'pdf';
	$config['max_size'] = '100000';
	$config['max_width'] = '1024';
	$config['max_height'] = '768';
	$this->load->library('upload', $config,'sj2upload'); // Create custom object for cover upload
	$this->sj2upload->initialize($config);
	$this->sj2upload->do_upload('filesj2');

	$update=$this->m_data->updatetambahansj($nosjawal,$nosjganti);
	if($update)
	{
		$this->m_data->updatefilesjtambah($namafile,$nosjganti);
		$this->session->set_flashdata('berhasilupdate', 'NO SJ TAMBAHAN :'.$nosjawal.' BERHASIL DIEDIT MENJADI '.$nosjganti.'!');
		redirect('dashboard/surat_jalan_tambahan');
	}
	else{
		$this->session->set_flashdata('gagalupdate', 'NO SJ TAMBAHAN :'.$nosjganti.' GAGAL DIUPDATE!');
		redirect('dashboard/surat_jalan_tambahan');
	}
